...
// Description: Allows user to add a fire station
// -->
<?php
//timezone
	...
	$newID;
	try {
		$command = "SELECT MAX(ObjectID) FROM FireStations LIMIT 1";
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST') {
		...
		$myObj['StationNo'] =  $_POST['StationNo'];
		...
		try {
			...
			if ((empty($myObj['stationID'])) || (empty($myObj['Name'])) || (empty($myObj['StationNo'])) || (empty($myObj['Address'])) || (empty($myObj['City'])) || (empty($myObj['Latitude'])) || (empty($myObj['Longitude']))) {
				$validInput = false;
				echo "<p>Failure</p>";
			} else {
				$command = "INSERT INTO FireStations VALUES (:stationID, :Name, :StationNo, :Type, :Address, :City, :Latitude, :Longitude)";
				$dbh = new PDO("mysql:host=sql103.epizy.com;dbname=epiz_23907369_000770411", "epiz_23907369", "M5CCTNczmAF");
				$stmt = $dbh->prepare($command);
